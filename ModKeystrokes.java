package frostclient.mods.impl.keystrokes;



import frostclient.utils.GuiUtil;
...
import frostclient.hud.ScreenPosition;
import frostclient.mods.AbstractMod;
import net.minecraft.client.Minecraft;
import net.minecraft.client.gui.Gui;
import net.minecraft.client.renderer.GlStateManager;
import net.minecraft.client.settings.KeyBinding;

public class ModKeystrokes extends AbstractMod {
	
	public static enum KeystrokesMode {
		
		WASD(Key.W, Key.A, Key.S, Key.D),
		WASD_MOUSE(Key.W, Key.A, Key.S, Key.A, Key.D, Key.LMB, Key.RMB),
		WASD_MOUSE_SBT(Key.W, Key.A, Key.S, Key.D, Key.LMB, Key.RMB, Key.SBT),
	...
	private static class Key {
		...
		public static final Key SBT = new Key("§m§l----§r", Minecraft.getMinecraft().gameSettings.keyBindJump, 1, 61, 58, 18, false);
	...
	private KeystrokesMode mode = KeystrokesMode.WASD_MOUSE_SBT;
			...
			for(Key key : mode.getKeys()) {
				...
				if(key.cps) {
					...
					if(key.getName().matches(key.LMB.getName())) {
						font.drawString("CPS: " + getCPS(), pos.getAbsoluteX() + key.getX() + key.getWidth() / 2 - textWidth / 2 , pos.getAbsoluteY() + key.getY() + key.getHeight() / 2 + 4, key.isDown() ? new Color(0, 0, 0, 150).getRGB() : new Color(255, 255, 255, 102).getRGB());
					}
					
					if(key.getName().matches(key.RMB.getName())) {
						font.drawString("CPS: " + getCPS2(), pos.getAbsoluteX() + key.getX() + key.getWidth() / 2 - textWidth / 2 , pos.getAbsoluteY() + key.getY() + key.getHeight() / 2 + 4, key.isDown() ? new Color(0, 0, 0, 150).getRGB() : new Color(255, 255, 255, 102).getRGB());
					}
